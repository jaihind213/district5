...
from radio_duck import connect, OperationalError, db_types, ProgrammingError
...
def test_no_query():
    ...
    with app.run("localhost", http_server_port):
        try:
            with connect(host="localhost", port=http_server_port, api="/v1/sql", scheme="http") as conn:
...
def test_cursor_description():
    ...
    with app.run("localhost", http_server_port):
        conn = connect(host="localhost", port=http_server_port, api="/v1/sql", scheme="http")
        cursor = conn.cursor()
        cursor.execute("select duck_type from pond")
        cursor_desc = cursor.description

        assert 2 == len(cursor_desc), "expecting 2 descrirption for 2 column selected"
        for string_col_desc in cursor_desc:
            assert 7 == len(string_col_desc), "expecting sequence of size 7 for each description" \
                                              ""
        string_col_desc = cursor_desc[0]
        assert string_col_desc[0] == "duck_type", "expected col name in cursor description to be 'duck_type'"
        assert string_col_desc[1] == db_types.get_type_code('STRING'), "expected col type in cursor description to be 'STRING'"

        num_col_desc = cursor_desc[1]
        assert num_col_desc[0] == "total", "expected col name in cursor description to be 'total'"
        assert num_col_desc[1] == db_types.get_type_code(
            'NUMBER'), "expected col type in cursor description to be 'NUMBER'"
...
def test_cursor_iteration():
    ...
    with app.run("localhost", http_server_port):
        with connect(host="localhost", port=http_server_port, api="/v1/sql", scheme="http") as conn:
            with conn.cursor() as cursor:
                assert -1 == cursor.rowcount, "testing rowcount before execution"
...
def test_with_server():
    ...
    with connect(host="localhost", port=8000, api="/v1/sql/", scheme="http") as conn:
        with conn.cursor() as cursor:
            #good query1
